================================================================
  QualtricsToLime — Qualtrics to LimeSurvey Converter
================================================================

  Converts Qualtrics Survey Files (.qsf) to LimeSurvey Structure
  files (.lss) with full reporting and verification.
...
================================================================
  ...
  QualtricsToLime/
  |
  |-- QualtricsToLime_v2.exe    The converter application
...
================================================================
  ...
  STEP 1 — INPUT ANALYSIS
    ...
    - Features detected: display logic, validation, piped text,
      JavaScript, choice randomization, branching, embedded data
  ...
  STEP 3 — VERIFICATION
    ...
    Checks performed:
     ...
     4. Answer options        — Choices accounted for as answers
                                or subquestions
...
================================================================
  ...
  The report contains all three sections (input, output,
  verification) for every file processed, plus an overall summary.
...
================================================================
  ...
  MC / MAHR                     M             Multiple Choice horiz.
...
================================================================
  ...
  If you have Python installed, you can run directly from source
  without the .exe:
  ...
  No external dependencies are needed — the project uses only
...
================================================================
  ...
  - Set up any embedded data fields as URL parameters
     (Survey settings > Panel integration)
...
================================================================
  ...
  - Timing questions have no LimeSurvey equivalent and are
    converted to display-only (X) placeholders

  - Custom JavaScript in Qualtrics questions is not transferred;
    these questions need manual review in LimeSurvey

  - Complex nested boolean display logic may produce simplified
    or approximate relevance equations with FIXME markers

  - Block randomizers are detected but not fully translated to
    LimeSurvey randomization groups

  - Some piped text patterns with loop references produce
    FIXME_LOOP_REF placeholders for manual review

  - The question count in the input report may differ from the
    output because the converter skips Trash/Unused blocks and
    counts Matrix sub-items differently

  - Side-by-Side (SBS) questions are converted to Array (F) as
...
================================================================
  ...
  "No .qsf files found"
    Make sure your files are in the "input" folder and have the
    .qsf extension. The converter does not look in subfolders.

  Question count mismatch warning
    This is usually expected. The QSF inspector counts all
    question elements including those in Trash blocks, while the
    converter only processes active blocks.

  Mandatory count mismatch
    The converter only transfers ForceResponse=ON. Other Qualtrics
    validation types (content validation, regex) are noted in the
    report but may not have direct LimeSurvey equivalents.

  FIXME markers in converted survey
    Search the .lss file for "FIXME" to find expressions that
    need manual adjustment in LimeSurvey's ExpressionManager.

  Windows SmartScreen warning
    The .exe is not code-signed. Click "More info" then
...
================================================================
  ...
  FULLY OFFLINE
    This application runs 100% offline. It does not connect to
    the internet, send data to any server, or require network
    access. All processing happens locally on your machine.

  NO DATA COLLECTION
    The converter does not collect, store, or transmit any
    personal data, survey responses, or usage analytics. Your
    survey files never leave your computer.

  NO EXTERNAL DEPENDENCIES
    The application uses only the Python standard library.
    No third-party packages are included that could introduce
    supply chain risks or phone-home behavior.

  LOCAL FILE ACCESS ONLY
    The application only reads from the "input" folder and
    writes to the "output" folder. It does not access any
    other files or directories on your system.

  SELF-CONTAINED EXECUTABLE
    The .exe is built with PyInstaller and bundles a Python
    interpreter with the source code. No installation is
    required and no files are written outside the application
    folder.

  SOURCE CODE INCLUDED
    The full source code is included in the "source" folder
